...
class D4XXToMAVLink(object):
    ...
    def __init__(self, args):
        ...
        self.FORMAT       = [rs.format.z16, rs.format.bgr8]     # rs2_format is identifies how binary data is encoded within a frame
        ...
        self.FPS          = 30
        self.DEPTH_RANGE_M = [0.1, 8.0]       # Replace with your sensor's specifics, in meter
        ...
        if self.filters[1][0] is True:
            self.filters[1][2].set_option(rs.option.min_distance, threshold_min_m)
            self.filters[1][2].set_option(rs.option.max_distance, threshold_max_m)
        ...
        if not self.connection_string:
            ...
            self.progress("INFO: Using default connection_string %s" % self.connection_string)
        else:
            self.progress("INFO: Using connection_string %s" % self.connection_string)

        if not self.connection_baudrate:
            self.connection_baudrate = self.connection_baudrate_default
            self.progress("INFO: Using default connection_baudrate %s" % self.connection_baudrate)
        else:
            self.progress("INFO: Using connection_baudrate %s" % self.connection_baudrate)

        if not self.obstacle_distance_msg_hz:
            self.obstacle_distance_msg_hz = self.obstacle_distance_msg_hz_default
            self.progress("INFO: Using default obstacle_distance_msg_hz %s" % self.obstacle_distance_msg_hz)
        else:
            self.progress("INFO: Using obstacle_distance_msg_hz %s" % self.obstacle_distance_msg_hz)
    ...
    def send_obstacle_distance_message(self):
        ...
        if self.angle_offset is None or self.increment_f is None:
            self.progress("Please call set_obstacle_distance_params before continue")
        else:
    ...
    def att_msg_callback(self, value):
        ...
        if self.debug_enable == 1:
            self.progress("INFO: Received ATTITUDE msg, current pitch is %.2f degrees" % (m.degrees(self.vehicle_pitch_rad),))

    # Listen to AHRS2 data: https://mavlink.io/en/messages/ardupilotmega.html#AHRS2
    def ahrs2_msg_callback(self, value):
        self.vehicle_pitch_rad = value.pitch
        if self.debug_enable == 1:
            self.progress("INFO: Received AHRS2 msg, current pitch is %.2f degrees" % (m.degrees(self.vehicle_pitch_rad)))
        ...
        for dev in devices:
            if dev.supports(rs.camera_info.product_id) and str(dev.get_info(rs.camera_info.product_id)) in DS5_product_ids:
                ...
                if dev.supports(name):
                    if not self.camera_name or (self.camera_name.lower() == dev.get_info(name).split()[2].lower()):
                        self.progress("INFO: Found device that supports advanced mode: %s" % dev.get_info(name))
    ...
    def realsense_enable_advanced_mode(self, advnc_mode):
        while not advnc_mode.is_enabled():
            ...
            self.progress("INFO: Advanced mode is %s" "enabled" if advnc_mode.is_enabled() else "disabled")
    ...
    def realsense_connect(self):
        ...
        config.enable_stream(self.STREAM_TYPE[0], self.DEPTH_WIDTH, self.DEPTH_HEIGHT, self.FORMAT[0], self.FPS)
        if self.RTSP_STREAMING_ENABLE is True:
            config.enable_stream(self.STREAM_TYPE[1], self.COLOR_WIDTH, self.COLOR_HEIGHT, self.FORMAT[1], self.FPS)
    ...
    def set_obstacle_distance_params(self):
        ...
        self.progress("INFO: Depth camera HFOV: %0.2f degrees" % self.depth_hfov_deg)
        self.progress("INFO: Depth camera VFOV: %0.2f degrees" % self.depth_vfov_deg)
        ...
        self.progress("INFO: OBSTACLE_DISTANCE angle_offset: %0.3f" % self.angle_offset)
        self.progress("INFO: OBSTACLE_DISTANCE increment_f: %0.3f" % self.increment_f)
        self.progress("INFO: OBSTACLE_DISTANCE coverage: from %0.3f to %0.3f degrees" %
        ...
        if self.obstacle_line_height_ratio < 0 or self.obstacle_line_height_ratio > 1:
            self.progress("Please make sure the horizontal position is within [0-1]: %s"  % self.obstacle_line_height_ratio)
            sys.exit()

        if self.obstacle_line_thickness_pixel < 1 or self.obstacle_line_thickness_pixel > self.DEPTH_HEIGHT:
            self.progress("Please make sure the thickness is within [0-self.DEPTH_HEIGHT]: %s" % self.obstacle_line_thickness_pixel)
            sys.exit()
    ...
    def find_obstacle_line_height(self):
        ...
        if self.vehicle_pitch_rad is not None and self.depth_vfov_deg is not None:
            delta_height = m.sin(self.vehicle_pitch_rad / 2) / m.sin(m.radians(self.depth_vfov_deg) / 2) * self.DEPTH_HEIGHT
    ...
    def distances_from_depth_image(self, obstacle_line_height, depth_mat, min_depth_m, max_depth_m):
    ...
    def run(self):
        try:
            ...
            self.progress("INFO: pyrealsense2 version: %s" % str(rs.__version__))
        except Exception:
            # fail silently
            pass
        ...
        self.mavlink_thread = threading.Thread(target=self.mavlink_loop, args=(self.conn, mavlink_callbacks))
        ...
        if self.enable_msg_obstacle_distance:
            sched.add_job(self.send_obstacle_distance_message, 'interval', seconds = 1/self.obstacle_distance_msg_hz)
            self.send_msg_to_gcs('Sending obstacle distance messages to FCU')
        elif self.enable_msg_distance_sensor:
            sched.add_job(self.send_distance_sensor_message, 'interval', seconds = 1/self.obstacle_distance_msg_hz)
            self.send_msg_to_gcs('Sending distance sensor messages to FCU')
        else:
            self.send_msg_to_gcs('Nothing to do. Check params to enable something')
            self.pipe.stop()
            self.conn.mav.close()
        ...
        glib_loop = None
        if self.RTSP_STREAMING_ENABLE is True:
            self.send_msg_to_gcs('RTSP at rtsp://' + self.get_local_ip() + ':' + self.RTSP_PORT + self.RTSP_MOUNT_POINT)
            Gst.init(None)
            self.gstserver = D4XXToMAVLink.GstServer(self.RTSP_MOUNT_POINT, self.COLOR_WIDTH, self.COLOR_HEIGHT, self.FPS)
        ...
        try:
            while not main_loop_should_quit:
                ...
                for i in range(len(self.filters)):
                    if self.filters[i][0] is True:
                        filtered_frame = self.filters[i][2].process(filtered_frame)
                ...
                self.distances_from_depth_image(obstacle_line_height, depth_mat, self.DEPTH_RANGE_M[0], self.DEPTH_RANGE_M[1])
                ...
                if self.debug_enable == 1:
                    # Prepare the data
                    input_image = np.asanyarray(self.colorizer.colorize(depth_frame).get_data())
                    output_image = np.asanyarray(self.colorizer.colorize(filtered_frame).get_data())
                    ...
                    display_image = np.hstack((input_image, cv2.resize(output_image, (self.DEPTH_WIDTH, self.DEPTH_HEIGHT))))
